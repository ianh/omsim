...
// is this atom being grabbed?  prevents output and consumption by glyphs.
static const atom GRABBED = 1ULL << 19;

// is this atom part of a van berlo's wheel?
static const atom VAN_BERLO_ATOM = 1ULL << 20;

static const atom VALID = 1ULL << 24;
static const atom REMOVED = 1ULL << 25;
...
static const atom ALL_BONDS = NORMAL_BONDS | TRIPLEX_R_BONDS | TRIPLEX_Y_BONDS | TRIPLEX_K_BONDS;
...
enum mechanism_type {
    ...
    // is this arm grabbing or not?  each of the 6 grabbing possible grabbing
    // directions are tracked using a separate bit.
    GRABBING_LOW_BIT = 1 << 21,
};

static const enum mechanism_type GRABBING = 0x3FULL * GRABBING_LOW_BIT;
...
static void apply_glyphs(struct solution *solution, struct board *board)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        case BONDING: {
            ...
            atom *b = get_atom(board, m, 0, 1);
            if (*a && *b) {
                *a |= bond_direction(m, 0, 1) & NORMAL_BONDS;
                *b |= bond_direction(m, 0, -1) & NORMAL_BONDS;
            }
            break;
        }
        case UNBONDING: {
        ...
        case TRIPLEX_BONDING: {
            ...
            if (*ky && *yr && (*ky & *yr & FIRE)) {
                *ky |= bond_direction(m, 1, 0) & TRIPLEX_Y_BONDS;
                *yr |= bond_direction(m, -1, 0) & TRIPLEX_Y_BONDS;
            }
            if (*yr && *rk && (*yr & *rk & FIRE)) {
                *yr |= bond_direction(m, -1, 1) & TRIPLEX_R_BONDS;
                *rk |= bond_direction(m, 1, -1) & TRIPLEX_R_BONDS;
            }
            if (*rk && *ky && (*rk & *ky & FIRE)) {
                *rk |= bond_direction(m, 0, -1) & TRIPLEX_K_BONDS;
                *ky |= bond_direction(m, 0, 1) & TRIPLEX_K_BONDS;
            }
            break;
        }
        case MULTI_BONDING: {
            ...
            if (*center && *a) {
                *a |= bond_direction(m, 0, -1) & NORMAL_BONDS;
                *center |= bond_direction(m, 0, 1) & NORMAL_BONDS;
            }
            if (*center && *b) {
                *b |= bond_direction(m, 1, 0) & NORMAL_BONDS;
                *center |= bond_direction(m, -1, 0) & NORMAL_BONDS;
            }
            if (*center && *c) {
                *c |= bond_direction(m, -1, 1) & NORMAL_BONDS;
                *center |= bond_direction(m, 1, -1) & NORMAL_BONDS;
            }
            break;
        }
        case DISPOSAL: {
...
static void create_van_berlo_atom(struct board *board, struct mechanism m, int32_t du, int32_t dv, atom element)
{
    ...
    *a = VALID | GRABBED | VAN_BERLO | element;
}
...
static atom *lookup_atom(struct board *board, struct vector query)
{
    ...
    while (true) {
        ...
        if (position.u == query.u && position.v == query.v)
            return a;
        index = (index + 1) & mask;
        steps++;
        if (index == (hash & mask))
...
static void ensure_capacity(struct board *board, uint32_t potential_insertions)
{
    ...
    if (n == 0)
        n = 16;
    while (n <= 3 * (board->used + potential_insertions))
        n *= 2;
    if (n == board->capacity)
        return;
...
{
    switch (bond_type) {
    case 1: return NORMAL_BONDS;
    case 2: return TRIPLEX_R_BONDS;
    case 4: return TRIPLEX_K_BONDS;
    case 8: return TRIPLEX_Y_BONDS;
    default: return 0;
    }
}
